...
# # Global variables
# desired_x = -1.2  # Desired x-coordinate
# desired_y = -0.498765  # Desired y-coordinate
# threshold = 0.1  # Distance threshold for considering the position reached
# position_reached = False
...
#         self.get_logger().info(f'x:{self.x:.2f}')
#         # Check if the position has been reached
...
# Global variables
desired_x = 2.0  # Desired x-coordinate
desired_y = 2.0  # Desired y-coordinate
threshold = 0.1  # Distance threshold for considering the position reached
position_reached = False
...
def odom_callback(odom_msg):
    ...
    # Compute the Euclidean distance from the current position to the desired position
    distance = ((desired_x - x) ** 2 + (desired_y - y) ** 2) ** 0.5
...
def go_to_position(self):
    ...
    # Create a Twist message to control the robot's velocity
    cmd_vel_msg = Twist()
    cmd_vel_msg.linear.x = 0.2  # Desired linear velocity
    cmd_vel_msg.angular.z = 0.0  # Desired angular velocity
